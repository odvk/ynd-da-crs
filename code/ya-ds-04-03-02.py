...
# Задача2 / 2
# Напишите функцию print_mean() (от англ. mean, «среднее»), которая считает и выводит на экран среднее значение по заданному столбцу. Функция должна принимать два аргумента – data (таблицу) и column (номер столбца). Результат должен печататься на экране с двумя знаками после запятой и без сопроводительного текста.
# После создания этой функции можно посчитать средние значения по большинству столбцов. Мы уже сделали это в прекоде.
...
def print_mean(data, column):
    ...
    for i in range(len(data)):
        column_sum += data[i][column]
    column_average = column_sum / len(data)
    print ('{:.2f}'.format(column_average))
    # < напишите код здесь >
