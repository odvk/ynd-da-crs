...
#Допишите функцию filter_without_genre() (от англ. filter without a genre, в значении «отфильтруй фильмы, не относящиеся к жанру»). Она должна выкидывать из таблицы все фильмы заданного жанра. Аргументы этой функции: таблица с фильмами и жанр, который нужно исключить. Исходные данные менять не надо — пусть функция возвращает новую, очищенную таблицу.
...
def filter_without_genre(data, genre):
    ...
    for row in data:
        genres = row[4]
        if not genre in genres: # напишите условие, что данного жанра нет в списке
            result.append(row)
    return result
